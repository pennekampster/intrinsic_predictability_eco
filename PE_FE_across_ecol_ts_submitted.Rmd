...
```{r GPDD, out.width='7in', fig.width=9, fig.height=9}
dd <- gpdd_data
...
ggplot(data=subset(dd_GPDD, MainID %in% sample(dd_GPDD$MainID, 12))) + 
...
dd_lab$MainID <- as.numeric(as.factor(paste0(dd_lab$source, "_", dd_lab$chemostat, "_", dd_lab$species)))+4471
dd_lab$type <- "lab"
...
```{r out.width='7in', fig.width=9, fig.height=9}
ggplot(data=dd1) + geom_line(aes(x=time, y=abundance, colour=species)) + 
...
```{r out.width='7in', fig.width=9, fig.height=9}
ggplot(data=dd2) + geom_line(aes(x=time, y=abundance, colour=species)) + 
  facet_wrap(~dilution_rate, scales = "free", ncol=3) + scale_y_log10()
```

### Becks et al. 2005 data
```{r out.width='7in', fig.width=9, fig.height=9}
ggplot(data=dd3) + geom_line(aes(x=time, y=abundance, colour=species)) + facet_wrap(replicate~chemostat, scales = "free", ncol=3) + scale_y_log10()
```
...
dd_ts$data <- lapply(1:nrow(dd_ts), function(x) 
  mutate(dd_ts$data[[x]], abundance_sc = as.vector(scale(abundance, center=T, scale=T))))  
} else {
dd_ts$data <- lapply(1:nrow(dd_ts), function(x) 
  mutate(dd_ts$data[[x]], abundance_sc = as.vector(scale(abundance, center=F, scale=F))))  
}
```
...
dd_ts$finite_prop <- unlist(lapply(1:length(dd_ts$N), function(x) sum(is.finite(dd_ts$data[[x]]$abundance))))/dd_ts$N
...
dd_ts$ties_prop <- unlist(lapply(1:length(dd_ts$N), function(x) sum(diff(dd_ts$data[[x]]$abundance)==0, na.rm=T)))/dd_ts$N
dd_ts$number_of_gaps <- unlist(lapply(1:length(dd_ts$N), function(x) sum(abs(diff(dd_ts$data[[x]]$time))>=2)))
```
...
dd_ts$data <- lapply(1:nrow(dd_ts), function(x) mutate(dd_ts$data[[x]], Date = ymd(paste0(as.character(time+1900), "-01-01"))))
...
```{r}
dd_ts <- dd_ts %>% mutate(PE = map_dbl(data, ~ tryCatch(PE(.$abundance, weighted=T, tau=1, word_length = 3, tie_method = "first"), error = function(err) NA)))

# uncomment to use other tie breaking methods when calculating PE
#dd_ts <- dd_ts %>% mutate(PE_average = map_dbl(data, ~ PE(.$abundance, weighted=T, tau=1, word_length = 3, tie_method = "average")))
#dd_ts <- dd_ts %>% mutate(PE_random = map_dbl(data, ~ PE(.$abundance, weighted=T, tau=1, word_length = 3, tie_method = "random")))
```
...
```{r}
ggplot(data=dd_ts, aes(x=PE, group=type, fill=type)) + geom_density(alpha=.3) + theme(legend.position = "bottom")
```
...
for (i in 1:nrow(dd_ts)){
  ...
  simplex_fit <- simplex(ts_select$abundance_sc, 
  ...
  smap_pred  <- s_map(ts_select$abundance_sc, lib = c(1, (length(ts_select$abundance_sc)-step_ahead)), pred = c( c((length(ts_select$abundance_sc)-step_ahead-E_hat_rho)),  length(ts_select$abundance_sc)),  stats_only = F, norm_type = c("L2 norm"), E = E_hat_rho, theta = theta_hat_rho)
  ...
  s_maps[[i]] <- smap_pred
   ...
   {plot(ts_select$time, ts_select$abundance_sc, type="l", xlab = "Time", ylab="Abundance" 
         , ylim = range(min(smap_pred$model_output[[1]]$pred, ts_select$abundance_sc, na.rm=T), max(smap_pred$model_output[[1]]$pred,ts_select$abundance_sc, na.rm=T)))
     points(smap_pred$model_output[[1]]$time, smap_pred$model_output[[1]]$pred, col=2,ylim =range(min(smap_pred$model_output[[1]]$pred,ts_select$abundance_sc, na.rm=T), max(smap_pred$model_output[[1]]$pred,ts_select$abundance_sc, na.rm=T)))
      lines(smap_pred$model_output[[1]]$time, smap_pred$model_output[[1]]$pred, col=2,ylim =range(min(smap_pred$model_output[[1]]$pred,ts_select$abundance_sc, na.rm=T), max(smap_pred$model_output[[1]]$pred,ts_select$abundance_sc, na.rm=T)))
...
```{r}
E_type <- ggplot(data=dd_ts, aes(x=E, group=type, fill=type)) + geom_density(position="identity", alpha=.3) + guides(alpha=F) + theme(legend.position = "bottom")
theta_type <- ggplot(data=dd_ts, aes(x=theta, group=type, fill=type)) + geom_density(position="identity", alpha=.3)+ guides(alpha=F) + theme(legend.position = "bottom")
...
```{r}
dd_ts$nrmse <- dd_ts$rmse/unlist(lapply(1:nrow(dd_ts), function(x) {max(dd_ts$data[[x]]$abundance_sc, na.rm=T) - min(dd_ts$data[[x]]$abundance_sc, na.rm=T)} ))
...
dd_ts_simple <- select(dd_ts, c(nrmse_sqrt, rmse_sqrt, PE, type, N, zero_prop, ties_prop, number_of_gaps, E, theta, source))

lmer_nrmse_all <- lmer(nrmse_sqrt ~ PE + type +  PE:type + N + zero_prop + ties_prop + E + theta + (1|source), data = dd_ts)
...
partial_df %>% select(Effect, Rsq, lower.CL, upper.CL) %>% mutate_if(is.numeric, round, digits=3) %>% knitr::kable()
```
...
plt <- ggplot() + geom_line(aes(y=dd_ts$data[[label_id]]$abundance, x=dd_ts$data[[label_id]]$time)) + 
  geom_line(aes(y=dd_ts$model_output[[label_id]]$pred, x=dd_ts$model_output[[label_id]]$time), colour="red") + xlab("Time") + ylab("Abundance") + ggtitle(paste0(x, ")")) 
}
...
dd_ts2 <- dd_ts[,c("nrmse", "PE", "source", "type", "label", "N", "zero_prop", "ties_prop", "E", "theta", "number_of_gaps")]
dd_ts2$nrmse_sqrt <- sqrt(dd_ts2$nrmse)
dd_ts2$type <- as.factor(dd_ts2$type)
lme_fit <- nlme::lme(nrmse_sqrt ~ PE * type + + N +  zero_prop + ties_prop + E + theta, random = ~ 1 | source, data = dd_ts2)
...
fig4 <- (p1 | p2 | p3 ) / {p4} / (p5 | p6 | p7)  + plot_layout(ncol = 1, heights = c(1, 2, 1), widths = c(1, 2, 1)) 
fig4
